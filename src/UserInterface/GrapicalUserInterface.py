...
from UserInterface.MenuInformation import MenuInformation
from Clocks.PomodoroClock import PomodoroClock
from Clocks.Clock import Clock
from Settings.Settings import MAX_DAILY_WORK_TIME, NORMAL_DAILY_WORK_TIME, POMODOROMINUTES, POMODOROSECONDS, PROGRAMM_TITLE, FRAMESIZE, MAX_SIZE, MIN_SIZE, TITLE_FONT_COLOR, PROGRAMM_VERSION, FONTDICT, BUTTON_STYLE, LABEL_STYLE_ACTIVE, LABEL_STYLE_FROZEN,TITLE_BACKGROUND_COLOR_FROZEN, WIDTH
...
class GraphicalUserInterface(Tk):
    ...
    def __init__(self,  
        ...
        self.pomodoro_object: object = pomodoroObject
        self.clock_object: object = clock_object
    ...
    def export_data(self) -> None:
        ...
        self.database_object.create_csv_file(filepath)
...
#################################################################
    ...
    def initializePomodoroGUI(self) -> None:
        ...
        if self.pomodoro_is_enabled == False:
    ...
    def managePomodoroTimer(self, initial: bool = False) -> None:
        ...
        if self.pomodoro_object.getPomodoroActive() == False:
            self.pomodoro_object.resetClock(minutes= POMODOROMINUTES, seconds= POMODOROSECONDS)
...
##################################################################
    ...
    def updateBackgroundColour(self) -> None:
        ...
        if MAX_DAILY_WORK_TIME > self.clock_object.getHours() >= NORMAL_DAILY_WORK_TIME:
            self.timeLabel.config(bg="yellow")
            self.timeLabelHeader.config(bg= "yellow")
            self.resultFrame.config(bg= "yellow")
        
        elif self.clock_object.getHours() >= MAX_DAILY_WORK_TIME:
    ...
    def stopCounting(self):
        ...
        if (self.pomodoro_is_enabled == True) and (self.pomodoro_object.getPomodoroActive() == False):
